...
namespace _2023_GC_A2_Partiel_POO.Level_2
{
    ...
    public class Character
    {
        ...
        public void ReceiveAttack(Character attacker, Skill s)
        {
            if (s == null) return;

            float factor = TypeResolver.GetFactor(s.Type, this.BaseType);

            CurrentHealth = (int)(CurrentHealth - (s.Power + attacker.Attack - Defense) * factor);
            if (CurrentHealth <= 0)
            {
                CurrentHealth = 0;
                return;
